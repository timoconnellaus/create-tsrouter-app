...
import { CODE_ROUTER, FILE_ROUTER, copyAddOnFile, packageManagerExecute, sortObject, writeConfigFile, } from '@tanstack/cta-core';
...
export async function createApp(options, { silent = false, environment, cwd, appName = 'TanStack', }) {
    ...
    const templateDirRouter = resolve(options.framework.baseDirectory, options.mode);
    let targetDir = cwd || '';
    if (!targetDir.length) {
    ...
    if (warnings.length > 0) {
        if (!silent) {
            environment.warn('The following will be overwritten:', warnings.join('\n'));
        }
    }
    if (options.toolchain === 'biome') {
